...
| Volltextsuche | **Postgres (`pg_trgm`)** für v1, **Meilisearch** als spätere Option | Algolia | Für v1 direkt über die schon vorhandenen Trigram-Indizes gelöst (`search_all`-RPC) — kein zusätzlicher Dienst, kein API-Key, sofort einsatzbereit. Meilisearch bleibt die geplante Ausbaustufe für bessere Facetten-UX und Skalierung bei größerem Datenvolumen; Algolia weiterhin verworfen (teurer, US-Hosting) |
| Karten | **flutter_map + OpenStreetMap** im App-Kern, Apple/Google Maps als externer Navigations-Deeplink | Google Maps SDK | Bewusste Entscheidung gegen Google Maps aus Kostengründen — keine Billing-Pflicht, kein API-Key-Management, keine Nutzungsgebühren pro Kartenaufruf. flutter_map ist reifes Open-Source-Clustering/Marker-Tooling; OSM-Kartendaten reichen für Städte-Discovery völlig aus. Nutzer bekommen weiterhin die Wahl, extern in Apple Maps oder Google Maps zu navigieren (reiner Deeplink, kein SDK nötig) |
